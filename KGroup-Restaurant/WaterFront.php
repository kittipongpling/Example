                            ...
                            <div class="content table-responsive table-full-width">
                                <div class="canvas">
                                    ...
                                    <table style="width:100%">
                                        ...
                                        <table width="100%" border="0" class="WaterFront">
                                            <tr>
                                                <?php 
                                                require_once("connection.php");
                                                $rn = 0;
                                                $PathIcon = "images/MantraPlan/TableIcon/";
                                                $sql = "SELECT * FROM (SELECT tablemantraplan.*,bookingres.BKStatus FROM tablemantraplan LEFT JOIN bookingres ON tablemantraplan.TableID = bookingres.TableID WHERE bookingres.DataIn = '$_SESSION[SelectDate]' AND bookingres.TimeRange = '$_SESSION[SelectTime]' AND tablemantraplan.TableZone ='4' AND tablemantraplan.TableGroup ='A' AND tablemantraplan.ResName ='KeereeTara' UNION All SELECT tablemantraplan.*,tablemantraplan.TableStatus FROM tablemantraplan LEFT JOIN bookingres ON tablemantraplan.TableID = bookingres.TableID WHERE TableZone ='4' AND TableGroup ='A' AND tablemantraplan.ResName ='KeereeTara') As TBid GROUP BY TBid.TableID ORDER BY TBid.Orders ASC";
