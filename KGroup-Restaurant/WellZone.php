                                ...
                                <div class="canvas">
                                    ...
                                    <table style="width:100%">
                                        ...
                                        <table width="100%" border="0" >
                                          <tr>
                                            <td>
                                              <table width="90%" border="0" class="WellZone">
                                                <tr>
                                                <?php 
                                                require_once("connection.php");
                                                $rn = 0;
                                                $PathIcon = "images/MantraPlan/TableIcon/";
                                                $sql = "SELECT * FROM (SELECT tablemantraplan.*,bookingres.BKStatus FROM tablemantraplan LEFT JOIN bookingres ON tablemantraplan.TableID = bookingres.TableID WHERE bookingres.DataIn = '$_SESSION[SelectDate]' AND bookingres.TimeRange = '$_SESSION[SelectTime]' AND tablemantraplan.TableZone ='3' AND tablemantraplan.TableGroup ='B' AND tablemantraplan.ResName ='KeereeTara' UNION All SELECT tablemantraplan.*,tablemantraplan.TableStatus FROM tablemantraplan LEFT JOIN bookingres ON tablemantraplan.TableID = bookingres.TableID WHERE TableZone ='3' AND TableGroup ='B' AND tablemantraplan.ResName ='KeereeTara') As TBid GROUP BY TBid.TableID ORDER BY TBid.TableID DESC";
                                          ...
                                          <tr>
                                            <td>
                                              <table width="90%" border="0" class="WellZone2">
                                                <tr>
                                                <?php 
                                                require_once("connection.php");
                                                $rn = 0;
                                                $PathIcon = "images/MantraPlan/TableIcon/";
                                                $sql = "SELECT * FROM (SELECT tablemantraplan.*,bookingres.BKStatus FROM tablemantraplan LEFT JOIN bookingres ON tablemantraplan.TableID = bookingres.TableID WHERE bookingres.DataIn = '$_SESSION[SelectDate]' AND tablemantraplan.TableZone ='3' AND tablemantraplan.TableGroup ='A' AND tablemantraplan.ResName ='KeereeTara' UNION All SELECT tablemantraplan.*,tablemantraplan.TableStatus FROM tablemantraplan LEFT JOIN bookingres ON tablemantraplan.TableID = bookingres.TableID WHERE TableZone ='3' AND TableGroup ='A' AND tablemantraplan.ResName ='KeereeTara') As TBid GROUP BY TBid.TableID ORDER BY TBid.TableID DESC";
                                                $query = mysqli_query($conn,$sql);
                                                while($result=mysqli_fetch_array($query,MYSQLI_ASSOC))
	                                               {
                                                    $NewVar = $result["TableNumber"];
                                                    ...
                                                    if($result["TableNumber"] == '36'){
                                                        echo '<td></td>';
                                                        $rn++;
                                                    }
                                                    if($result["TableNumber"] == '31'){
                                                        echo '<td></td><td></td>';
                                                        $rn++;
                                                    }
                                                     if($result["TableNumber"] == '30'){
                                                        echo '<td></td>';
                                                        $rn++;
                                                    }
                                                    echo '<td>';
